...
**A screen that expires presses its own first control.** `timeout` carries a duration and nothing
else.
...
## `then` is gone and nothing was lost

A timeout used to carry `then: Screen` beside its duration. That was a second description of what the
screen's own control already did: the run-over screen's *Again* goes to the title and its timeout went
to the title, and the two agreed only because somebody kept them in step.

Expiring now presses the first control. It costs a rule — **a screen that expires must have a control
to press**, guarded — and it buys the thing that made this decision possible: *Onward* carries the run
into the next level, which is not a `Screen` value and could never have been named by one.

## Confirmed, not assumed

Probes in `scripts/probes/0063-respite.mjs`. **7 red, and every tree back to what it was copied as.**
...
| a screen that expires with no control for the expiry to press | `a screen that expires has a control for the expiry to press` |
