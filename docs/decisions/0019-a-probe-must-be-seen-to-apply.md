...
**Added 2026-08-05**, after a set of nineteen probes — every one of them red on demand — stood over a
constant whose *units were wrong*, and the code shipped unusable.

A probe proves a guard **fires on the bug the probe describes**. That is the claim, and it is a real
one. It is not the claim that the guard measures the right thing.

The failure looks like this. The guard asserted *"N pixels of finger produce a deflection of 1"*. The
probe broke the conversion and the guard went red, correctly. But the deflection was defined in terms
of the very constant being guarded, so **the assertion holds at any conversion factor** — including
one requiring five metres of thumb to cross the screen. The break and the guard were written in the
same wrong units and agreed with each other perfectly.

⚠️ **A break and its guard share an author and a vocabulary, and that is the blind spot.** Everything
this file does is check that they agree. Two things written in the same wrong units agree.

What closes it is not another probe. It is an assertion written in the units the *player* experiences
— see [0027](0027-measure-the-picture-not-the-model.md), which now carries that rule and the two
cases behind it.

**So the pair, stated plainly:** 0019 catches a guard that does not fire. 0027 catches a guard that
fires on the wrong quantity. Neither substitutes for the other, and this repository had only the
first for a day.
