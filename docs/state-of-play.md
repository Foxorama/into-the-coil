...
| ~~a death throws back one piece per kind, carrying the count, holding its face, wearing ×N~~ — **reversed by 0372** | [0243](decisions/0243-a-death-throws-back-one-piece-per-kind.md) → [0372](decisions/0372-a-death-keeps-the-ladders.md) |
...
| **the serpent strikes: a phase says what it throws; acid and void, two shots in two inks; lightning in columns with warning lines** | [0248](decisions/0248-the-serpent-strikes.md) |
...
| ~~a death takes both ladders and both kinds and throws every rung where the ship died~~ — **reversed by 0372** | [0266](decisions/0266-a-death-throws-the-ladders-back.md) → [0372](decisions/0372-a-death-keeps-the-ladders.md) |
| **a death and a continue keep the ladders, the kinds and the charges; the bomb pickup and the clear's charge are gone; a thrown bomb lands a twentieth of a boss; a gun's hit on a boss is weighed by its row and a boss may author its own** — the first of four on one ask, in [`the-arsenal-planned`](../reports/the-arsenal-planned-2026-09-26.md) | [0372](decisions/0372-a-death-keeps-the-ladders.md) |
| **a full ladder buys its own face's special; the arsenal is a stack one trigger throws newest-first; the pulse's overdrive and the seeker's hunt are surges, a timed aura that strengthens a weapon** — the second of four | [0373](decisions/0373-a-special-is-the-guns-own.md) |
| **the arc's storm strikes six bodies on the screen, chains twice from each and flickers; the shuriken's whirlpool turns and grows until none of it is on the screen, landing on a boss again and again; the entity ceiling is 614** — the third of four | [0374](decisions/0374-the-storm-and-the-whirlpool.md) |
| **the auto-gun buys the bomb and the forward missiles the golden surge; the bomb is a large missile that goes off as a filled three-picture explosion, on a launch and a boom rebuilt on the root; a third of a second between throws for the flash cap** — inserted on the play | [0375](decisions/0375-the-bomb-is-a-missile.md) |
| **two triggers: the gun's specials on the first and the tubes' on the second, each its own newest-first stack; two touch buttons** — corrects 0373's single queue; the void is next, on the tubes' trigger | [0376](decisions/0376-a-trigger-for-the-gun-and-one-for-the-tubes.md) |
| **a shield at a full shell is a void missile: a rift that removes shots, bodies and boss lightning, carves stone for the level and lands a tenth of a boss; a rift is its own picture in the blast pool; a thrown special that reaches the screen's edge goes off there** — the last of the ask; a play is owed | [0377](decisions/0377-the-void.md) |
| **every special is heard as itself: a press and a going-off named on its row, seven new cues on the root; the rig lays every cue in its own width through the panner's stereo law — it had rendered all of them narrower than the game — and `--play` has a take of every special over the score** — the ear is owed | [0378](decisions/0378-the-specials-are-heard.md) — amends [0209](decisions/0209-the-rig-hears-in-stereo.md) |
| **the specials are seen: thrown ones fly at 1.5 and draw bigger, the balls trailing their light; the storm and the whirlpool last half a second longer; a tube special fires two missiles of its own from two pods on the flanks, which are its picture in place of the circle; the missile pool is 40** — a play is owed | [0379](decisions/0379-the-specials-are-seen.md) — amends [0373](decisions/0373-a-special-is-the-guns-own.md) |
...
| **a hit flash is a WASH over the sprite and not a cutout of it** | [0278](decisions/0278-the-flash-is-a-wash.md) |
| **a cheap mechanism does not rename the ask: a scale-pulse is not animation, and the thing that was asked for is what the guard is written against** | [0280](decisions/0280-a-cheap-mechanism-does-not-rename-the-ask.md) |
| **a mechanism whose output is identical for every kind is a CONSTANT, and *no row can forget it* argues for a default rather than one; a change is finished when the thing it added can differ per instance** | [0282](decisions/0282-a-mechanism-for-every-instance-makes-them-one-instance.md) — supersedes [0281](decisions/0281-a-boss-guards-its-own-back.md), whose code it removed |
| **the serpent is a CHAIN: a row states a body as girths, a step, a sway and a lag; nodes are placed in along-space with a travelling wave, and a hit anywhere on it is a hit on the animal** | [0283](decisions/0283-the-serpent-is-a-chain.md), [`the-chain-played`](../reports/the-chain-played-2026-09-09.md) |
| **the head is a serpent's: horns, a brow plate, a gape that is a notch in the silhouette and four fangs that are teeth rather than decals; a body starts where the head ENDS** | [0284](decisions/0284-the-head-is-a-serpents.md) |
| **the mouth is alive: a face is a frame and which frame is the fight's; which jaw a mark belongs to is authored, never deduced; a snap answers the PLAYER and a tell answers the fight, and they throw the jaw opposite ways** | [0285](decisions/0285-the-mouth-is-alive.md) |
| **a serpent runs off the screen: a boss may be longer than the view and its HULL is what has to stay on it; a PLACED entity is not culled; 0022's worst case is 515 and a boss with a body is a line on its list** | [0286](decisions/0286-a-serpent-runs-off-the-screen.md) — reopens the sizing half of [0022](decisions/0022-frame-rate-is-a-feature.md) on [0153](decisions/0153-desktop-is-the-target.md)'s argument |
| **a cavity is not washed: a flash lights a body and does not get into what the body is NOT, so the hit wash is the tile with the kind's cavities taken out of it in one fill** | [0287](decisions/0287-a-cavity-is-not-washed.md) — amends [0278](decisions/0278-the-flash-is-a-wash.md) |
| **the skull is longer: bigger and longer are two numbers; a HURTBOX is the size of the head that is drawn, held to the skull's own two axes; a transform at the point of drawing beats re-typing the art** | [0288](decisions/0288-the-skull-is-longer.md) |
| **a bob can REAR: a bob has an along half, phase-locked so the hull sweeps an arc; it is a term of the STATION and never an arm of the move switch; a hull that lunges stands further back, because the room comes out of the player's** | [0289](decisions/0289-a-bob-can-rear.md) |
| **the acid is SERPENTINE: a spray whose beads lie on a wave that stretches as it flies — `whip`'s mechanism with the heading waving instead of the speed; a shape's own bead count rides the ATTACK and not the phase, because a wave cannot be drawn with three points** | [0290](decisions/0290-the-acid-is-serpentine.md) |
...
| **the fish has FOUR STAGES: the two the play liked lead — the rake with kites, kindled, then the summons with the shoal, ablaze — and two new ones follow: the breaker roaming anywhere along the near edge with a half-second tell and the field otherwise empty, then white-hot, leaping out through the edge and back across the screen on a clock of its own between whips of flame; a `leap` on a phase, `roams` and `warning` on a breaker, a third look in the ember's core inks** | [0380](decisions/0380-the-fish-has-four-stages.md) — amends [0315](decisions/0315-the-fish-throws-a-breaker.md), [0317](decisions/0317-the-pressure-comes-forward.md), [0320](decisions/0320-the-fish-kindles.md) |
| **the fish is BIGGER, and its tail is one animal: 50 across where it was 42, the hurtbox with it; the fin's base reaches forward under a shorter stump so the join is under the flesh, the beat slower and shallower, and no yaw on the hull** | [0381](decisions/0381-the-fish-is-bigger.md) — amends [0374](decisions/0374-the-fish-beats-its-tail.md) |
...
each tier**, since the player's three numbers are the whole of the shape. What a tier's
*narrowest* means at a fork was asked in
[`the-night-of-2026-09-21`](../reports/the-night-of-2026-09-21.md) and answered with its
recommendation. Item 5 is landed — [0351](decisions/0351-rime-shelf-is-ice.md), Rime Shelf — **owed a
play**; the floor holds its white far below white, and the player chose to keep the floor and adjust
...
rakes, and its weapons are cumulative, taking turns as the hydra's heads do. **The seventh is
landed** — [0262](decisions/0262-the-eagle-throws-quills.md): the eagle's bullet is a quill of its
...
happening"* — a spawn rule rather than a re-authoring, because a wave's `at` is a place and a fight's
length is the player's; [0268](decisions/0268-the-bob-keeps-its-centre.md), a bobbing hull walking out
of the lane at every phase change; and
[0269](decisions/0269-a-mid-boss-is-fought-for-as-long-as-its-level-says.md) — *"mid bosses need less
health… short nasty fights"* — three phases each on the real boss's tempo, and the health solved to a
ladder in seconds. `scripts/weigh-fight.mjs` is the instrument for the last two.

⚠️ **AND THE HALF THAT COULD NOT BE BUILT NOW CAN BE, BECAUSE 0269 BUILT IT** — *"some [firing waves]
after miniboss."* Two levels' mid-boss fights used to outlive their own wave scripts, so there was no
...
⚠️ **AND THE FLIGHT OF THE WAVES WAS REVIEWED ON 2026-09-26** —
...
look — [0305](decisions/0305-the-serpent-darkens.md); and the entrance —
...
wall's reveal against the *"no indication"* report, the bar over a bright place, the readout on a
phone. Each decision's *what this leaves owed* names its own question.
...
built; **the adds are** — [0373](decisions/0373-the-fish-spits-its-adds.md), on the ask of
2026-09-26 rather than the plan's arc: spat out of the mouth, hunting and firing — and the fight
length is not. The same ask landed a tail that beats
([0374](decisions/0374-the-fish-beats-its-tail.md)) and a breach with a body
([0375](decisions/0375-the-breach-has-a-body.md)). **Played on 2026-09-27**: *"fish is better… last
stage is great, it should be stage 2, the stage before it should be stage 1 and we need a new stage
3 and four… the graphic needs to be a bit bigger… the animation is a bit funky and weird… definitely
not a high quality boss at the moment"* — answered by
[0380](decisions/0380-the-fish-has-four-stages.md) (four stages, the liked two first, a roaming
warned breaker and a white-hot leaping fourth) and [0381](decisions/0381-the-fish-is-bigger.md)
(a fifth bigger, the tail's join under the flesh, no yaw). **Both are owed a play**, and 0380's own
table says how much harder the fight is to stand in now, which the play owns.
...
| the adds, since 0321 | both have a halo now, and a halo is the one mark that gets BIGGER relative to its body as the body shrinks. Do two dozen on the screen at once read as a shoal or as a haze? The last phase has kites and minnows together |
...
| **the spat adds, since 0373** | do a fan of kites bursting out of the mouth and diving back read as the fish attacking, or as free kills in the gun line? The blink while they clear the spray — does it read as *not yet* or as a hit that did nothing? And the fight is **0.69 hits a second parked, 0.39 sweeping**, where it was 0.15: pressure, or a wall on the second boss of seven — 0373's own table |
| **the tail, since 0374** | does the beat read as swimming at 42 units on a starfield, and does the join at the peduncle show when it sweeps? Measured on the outline, never seen in flight |
| **the breach's sound, since 0375** | the ear that called it terrible is owed the new one: `node scripts/hear.mjs --only=bossBreach,bossSpit --place=nebula` |
| **the four stages, since 0380** | does the fight read as escalating from a kindled opening, and is the roaming breaker's half-second of spines-in-the-edge enough of a tell to move for? The leap: does the dive out through the edge read as a leap and not a vanish, and does the arrival after it read as the same fight? **Parked, the pulse takes 0.88 hits a second where it took 0.15 before the adds were spat** — pressure or a wall on the second boss of seven |
| **the size and the tail, since 0381** | 300 CSS pixels across on 1280×720 where it was 252; the join is measured on the outline and photographed on the bench, never in a play. Is the slower, shallower beat still *funky*, or is the funkiness gone with the yaw? |

### ⚠️ THE ART PASS IS DONE, IN FOUR — AND NOT ONE OF THEM HAS BEEN PLAYED
...
| ~~the shoal and the kites~~ | landed as [0321](decisions/0321-the-shoal-is-drawn.md). **The interesting half is what does not survive the scale**: at 6.5 and 5 units 0106's floor is 0.13 and 0.17 of the drawing radius, so rays, lines and motes cannot exist — what lifts is the halo and the form-shade. The kite was drawn as a curve, photographed and put back, because it spends one of the two channels 0314 separates the pair on. **Shot on the sheet; never played** |
...
faces. **And 0240 was played, and [0241](decisions/0241-the-ship-wears-its-colours.md) answers all
four items of [`the-ship-played`](../reports/the-ship-played-2026-09-05.md)** — the arc cut back
again, the seeker out of the ship's ink, a livery on the hull, a flame that leans rather than
...
[`the-scatter-under-fire`](../reports/the-scatter-under-fire-2026-09-05.md). **And the coil was
played and was not the ask: [0244](decisions/0244-a-blade-rides-a-helix.md) makes the blades a
helix** — [`the-helix-asked`](../reports/the-helix-asked-2026-09-05.md): two strands from the
...
game has. **The serpent's is landed — [0248](decisions/0248-the-serpent-strikes.md): a phase says
what it throws, acid and void in their own inks, and lightning in columns that warn first. **And the
eagle's — [0249](decisions/0249-the-eagle-summons.md): a whip that bows as it flies, and summons
that put kites and raptors on the field. And the pterodactyl's lasers —
...
taking turns a volley. And the jellyfish's — [0255](decisions/0255-the-jellyfish-opens.md):
pulsing tendrils, moon jellies falling from three quarters, and an opening that keeps throwing
void. **Every real boss in the brief has its first iteration now; what is next is the play** —
seven fights on the branch previews, each decision's *owed* list read against what the player
saw. What is owed is
...
and their ladders are starting points. The bench takes `?weapon=arc`
