...
| **the auto-gun buys the bomb and the forward missiles the golden surge; the bomb is a large missile that goes off as a filled three-picture explosion, on a launch and a boom rebuilt on the root; a third of a second between throws for the flash cap** — inserted on the play; the void is next, then the trigger rework that splits the gun's specials from the tubes', and the specials' own cues and the player's ear on these two are owed | [0375](decisions/0375-the-bomb-is-a-missile.md) |
