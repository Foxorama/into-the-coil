...
const WITHOUT_PROBES: Record<string, string> = {
  ...
  /*
    ⚠️ **0243's ROW SAT HERE FOR ONE DAY, AND THE ASSERTION BELOW IS WHAT DELETED IT.** 0256 removed
    the scatter, the `stack` field and the badge, so 0243's four probes went with
    `tests/stack.test.ts` and this table carried the reason.
    `docs/decisions/0266-a-death-throws-the-ladders-back.md` put all three back; the probes and the
    suite came back with them, and an exemption whose code exists again is exactly the stale excuse
    *no exemption outlives its probes* refuses to let sit. It went red on the restore, which is the
    only reason this comment is here rather than the row.
  */
