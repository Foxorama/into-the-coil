...
import { BURN_ASK, EASE_ASK, PULSE_STEPS, SWAY, THRUST, THRUST_KINDS } from '../src/content/exhaust.ts';
...
describe('0230 — the ship flies', () => {
  ...
  it('burns on a hard push forward and eases on a hard pull back, read off the ask', () => {
    ...
    frame.step();
    expect(THRUST.idle.frames, 'the idle flame is not what a resting ship shows').toContain(w.exhaust.at(0).sprite);
    ask.along = 1;
    frame.step();
    expect(THRUST.burn.frames, `a hard push shows ${drawn(w)} rather than a burn`).toContain(w.exhaust.at(0).sprite);
    ask.along = -1;
    frame.step();
    expect(THRUST.ease.frames, `a hard pull shows ${drawn(w)} rather than an ease`).toContain(w.exhaust.at(0).sprite);
    // And a nudge is neither: half deflection idles.
    ask.along = BURN_ASK / 2;
    frame.step();
    expect(THRUST.idle.frames, 'a nudge forward burns').toContain(w.exhaust.at(0).sprite);
    ask.along = EASE_ASK / 2;
    frame.step();
    expect(THRUST.idle.frames, 'a nudge back eases').toContain(w.exhaust.at(0).sprite);
  });
  ...
  it('still burns with the ship pinned against the front of its box, because the ask is the state', () => {
    ...
    */
    ...
    expect(THRUST.burn.frames, `pinned and pushing, the engines show ${drawn(w)}`).toContain(w.exhaust.at(0).sprite);
  });
  ...
  it('pulses: a pulsing state alternates its frames on the step clock', () => {
    ...
    expect(seen.size, 'the idle flame never changes, so it is a sticker and not a thruster').toBe(THRUST.idle.frames.length);
  ...
  it('sways: the flame hangs against the ship’s sideways velocity, and swings back when it stops', () => {
    const { world: w, frame, ask } = piloted();
    frame.step();
    ...
    // Climbing (across falling), the flame hangs BELOW the tail: further across than the ship.
    expect(w.exhaust.at(0).across, 'the flame does not hang against a climb').toBeGreaterThan(w.ship.across);
    expect(w.exhaust.at(0).across - w.ship.across).toBeCloseTo(-w.ship.velAcross * SWAY, 6);
    ask.across = 0;
    for (let i = 0; i < 120; i++) frame.step();
    expect(Math.abs(w.exhaust.at(0).across - w.ship.across), 'the flame never swings back').toBeLessThan(0.05);
  });
  ...
  it('is drawn under the shell and the ship and over every shot, and every thrust row has frames and a trail', () => {
    ...
    */
    ...
    for (const kind of THRUST_KINDS) {
      ...
      expect(row.frames.length, `${kind} has no frames`).toBeGreaterThan(0);
      expect(new Set(row.frames).size, `${kind} lists a frame twice`).toBe(row.frames.length);
      expect(row.trail, `${kind}'s flame is not behind the ship`).toBeGreaterThan(0);
    }
    expect(THRUST.idle.frames.length, 'the idle flame cannot pulse').toBeGreaterThan(1);
    expect(THRUST.burn.frames.length, 'the burn cannot pulse').toBeGreaterThan(1);
    // A burn is longer than an idle, which is the whole of what a hard push shows.
    const longest = (kind: 'idle' | 'burn'): number => Math.max(...THRUST[kind].frames.map((s) => SPRITE_EXTENT[SPRITE_KINDS[s]!]));
