// The hold: what scale brings each rung of each place to where its contour says, against its `run`.
//
// docs/decisions/0226-the-level-holds-one-loudness.md, as
// docs/decisions/0329-a-level-may-fall.md leaves it.
//
...
// reads the same function with the hold ON and asks that every rung sit within HOLD_BAND_DB of where
// its contour puts it.
//
// ⚠️ THE TARGET IS `run` PLUS THE PLACE'S CONTOUR, AND IT WAS `run` FOR EVERYTHING — 0329. A place may
// author how far BELOW its opening a rung sits, in LU, because that is the unit the report was made in
// and the compressor halves every move stated as a gain. Absent is zero, which six places state.
//
...
import { THEME_KINDS, contourOf } from '../src/content/themes.ts';
...
for (const theme of places) {
  const run = loudAt(theme, 'run', 1);
  const row = [];
  const said = [];
  for (const rung of rungs) {
    // Where this place says this rung sits — its `run`, less whatever its contour states (0329).
    const target = run + contourOf(theme, rung);
    ...
    const wanted = contourOf(theme, rung);
    said.push(
      `${rung} ${(20 * Math.log10(trim)).toFixed(2)} dB → ${loud.toFixed(2)} LUFS ` +
        `(${(loud - run).toFixed(2)} of a wanted ${wanted.toFixed(2)})`,
    );
  }
  console.log(`  ${theme}: { ${row.join(', ')} },`);
  console.error(`  ${theme}: run ${run.toFixed(2)} LUFS; ${said.join('; ')}`);
}
console.log('};');
