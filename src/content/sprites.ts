...
export const SPRITE_EXTENT: Record<SpriteKind, number> = {
  ...
  */
  ...
  arcNode: 2.4,
  // A blade: a tenth of the lane, since 0244 — played at 3.2, *"shuriken stars need to be a lot
  // bigger"*; at 7, *"bigger and steel coloured, also with a bit of a glow to them"*; and at 12,
  // *"a bit smaller, they take up a lot of visual screenspace and make it hard to see enemies and
  // enemy fire"*. It is the one shot the player has to read the PATH of, and the path runs to the
  // edge of the screen; `THE SIZE` in `tests/blades.test.ts` holds the box under a tenth of the
  // lane. The star is drawn at four fifths of this (`BLADE_GLYPH` in `src/render/bake.ts`) and the
  // rest is its glow; the hurtbox is the row's, and `tests/combat.test.ts` holds it inside what is
  // drawn.
  shuriken: 10,
  shurikenTurn: 10,
