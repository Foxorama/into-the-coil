...
/*
  ── A BOSS GUARDS ITS OWN BACK — 0281 ───────────────────────────────────────────────────────────

  `docs/decisions/0281-a-boss-guards-its-own-back.md`. Reported from play: *"like all bosses, you can
  fly behind it with the lightning canon and just basically sit their nuking it because it doesn't go
  backwards at all."*

  ⚠️ **THE REPORT NAMES THE MOVEMENT AND THE CAUSE IS THE AIM**, which is measured in
  `reports/the-serpent-played-2026-09-08.md`: `PLAYER_LEAD` is 167.1 and every boss's
  `station + drift + radius` is 149 to 155, so there is a corridor 12 to 18 units deep UP-LANE of
  every hull that the ship can reach — and `spray`, `rake`, `wall`, `whip` and `beam` are all centred
  on `π`, which is down-lane. Five of the nine arms cannot reach the ground behind their own boss at
  any spread, and four of the seven real bosses throw none of the two that can.

  ⚠️ **AND THIS IS NOT THE BOSS TURNING ROUND, WHICH IS SAID PLAINLY BECAUSE
  `docs/decisions/0280-a-cheap-mechanism-does-not-rename-the-ask.md` IS ABOUT EXACTLY THIS.** A hull
  that faced the player would need a heading, and `src/render/scene.ts` says *"`blit`
  cannot rotate"*; a hull that reversed into the corridor would need a station past the 160
  `tests/level.test.ts` allows, which is the number that keeps the whole hull on the narrowest view
  (0061). Both are the segment chain's territory. What this buys is that **the ground behind a boss
  is not free** — it does not buy a boss that turns to face you, and the decision says so.
*/

/**
 * How many shots a lash is.
 *
 * ⚠️ **SEVEN, AND THREE WAS MEASURED AND WAS NOT ENOUGH.** A lash is a fan out of one point, so what
 * it covers is an ANGLE and what the player stands in is a LANE — a hundred units wide, at a range
 * that is sometimes twenty. Three rays left gaps a parked ship sat in: over twenty seconds the hydra
 * never once touched a ship standing at 15 or at 85 across, and the worst first hit anywhere in the
 * game was **15.8 seconds**. At seven, with the sweep below, nothing anywhere goes untouched and the
 * worst first hit is **5.5 seconds**.
 *
 * ⚠️ **AND IT IS NOT AN ANGULAR-DENSITY PROBLEM ALONE, WHICH IS WHY THE SWEEP IS ALSO NEEDED.** Both
 * were measured separately and neither is sufficient: seven fixed rays still leave seven gaps.
 */
const TAIL_SHOTS = 7;

/**
 * How wide a lash is in total, in radians — 137°, so the seven sit every 23° from −69° to +69° of
 * straight up-lane, before the sweep moves them.
 *
 * ⚠️ **WIDE, BECAUSE THE CORRIDOR IS SHORT AND WIDE.** Measured: a ship parked at the up-lane wall
 * stands about 42 units behind the hydra and up to 53 units across from it, which is 52° off the
 * straight-back bearing — and the arc reaches 98 units at its cap, so *behind the boss* is a band as
 * wide as the lane rather than a line. A tight fan would cover one bearing out of a hull whose back
 * the ship can stand beside as easily as behind.
 *
 * ⚠️ **AND NOT WIDER, BECAUSE A RAY NEAR 90° DOES NOT GO BACKWARDS AT ALL.** `cos(69°)` is 0.36, so
 * the outermost rays climb up-lane at a third of `TAIL_SPEED` and cross the lane at the rest — which
 * is a wake that spreads sideways as it rises, and is why they are worth having. Past about ±75° they
 * stop rising at all and buy coverage on paper only.
 */
const TAIL_SPREAD = 2.4;

/**
 * How far a lash's whole fan swings behind the hull, in radians, and how far it steps each time.
 *
 * ── WHY A SWEEP AT ALL, WHICH IS A MEASUREMENT AND NOT A FLOURISH ───────────────────────────────
 *
 * ⚠️ **FIXED BEARINGS LEAVE FIXED GAPS, AND A PARKED SHIP SITS IN ONE.** Measured with the first
 * draft, which threw at −60°, 0 and +60° and never moved them: over twenty seconds at the up-lane
 * wall, the hydra never touched a ship standing at 15 or at 85 across — it was at 52°, squarely
 * between two rays. A fixed safe bearing behind a boss is the reported defect again, one bearing
 * narrower, and the player finds it once and parks there for the rest of the run.
 *
 * ⚠️ **A SAWTOOTH AND NOT A ROTATION.** `firePhase` turns for ever, which is right for a rake and
 * wrong here: a fan that kept turning would eventually point down-lane and the lash would stop being
 * a lash. This wraps into a fixed arc behind the hull, so every bearing in it is visited and none
 * outside it ever is.
 *
 * ⚠️ **THE SWING IS THE RAY GAP, AND THE STEP DOES NOT DIVIDE IT.** `TAIL_SPREAD / (TAIL_SHOTS − 1)`
 * is 0.4 of a radian, so a swing of 0.42 is just over one gap and the rays between them are exactly
 * what gets covered. And 0.17 into 0.42 walks the arc rather than landing on two or three places and
 * repeating — a step that divided the swing would be a coarser version of the same fixed bearings.
 */
const TAIL_SWING = 1.2;
const TAIL_TURN = 0.29;

/**
 * How often a boss lays a lash, in steps — every 0.9 s.
 *
 * ⚠️ **ITS OWN CLOCK, AND NOT THE PHASE'S.** `src/sim/entity.ts`'s `tailIn` carries what sharing
 * `fireIn` cost: the wake would inherit `fireGapFor`'s tier scaling and arrive twice as often at
 * `burn`, which took eleven of the fourteen fights to a full hostile pool. A wake is a property of
 * the hull being there.
 *
 * ⚠️ **AND IT IS A POOL BUDGET AS MUCH AS A CADENCE** —
 * [0192](../../docs/decisions/0192-a-guard-holds-an-invariant.md) says a budget names who owns its
 * number. `TAIL_SHOTS × life ÷ this` is how many lashes are alive at once, and at `TAIL_SPEED` a
 * lash crosses the corridor in about a hundred and fifty steps: seven every fifty-four is about
 * twenty of the hundred and fifty `CAPACITY.enemyShots` allows. `tests/crowd.test.ts` owns the
 * ceiling and is what says whether this is affordable; it went red twice on the way to this number.
 */
const TAIL_EVERY = 54;

/**
 * How long a lash goes on living once it has reached the wall, in steps — half a second.
 *
 * ── WHY A FUSE AT ALL, WHICH IS A POOL MEASUREMENT AND A PICTURE AT THE SAME TIME ───────────────
 *
 * ⚠️ **WITHOUT ONE A LASH LIVES 470 STEPS, AND ONLY THE FIRST 200 OF THEM CAN TOUCH ANYTHING.** The
 * leading cull is `cullLeadingAlong` — `MAX_ALONG_SPAN` plus two margins, about 240 units out, sized
 * for the widest device — and the deepest corridor in the game is the 44 units between the
 * jellyfish's hull and `PLAYER_LEAD`. Everything past that is a bullet climbing away from a place the
 * ship may not go, holding a pool slot: measured, that is 46 lashes alive on the jellyfish's last
 * phase, and `tests/crowd.test.ts` went red at a full 150.
 *
 * ⚠️ **AND IT IS ALSO THE RIGHT PICTURE**, which is why it is a fuse rather than a tighter cull. A
 * cull at the player's own wall would pop a bullet out of existence in the middle of an ultrawide
 * screen — [0036](../../docs/decisions/0036-an-event-the-model-knows-about-the-picture-mentions.md)
 * exactly. A wake that thins out and disperses behind the animal is what the thing IS, and 0263
 * already draws it: `BURST.melt`, *the melt*.
 *
 * ⚠️ **AND THE FUSE IS SOLVED PER LASH RATHER THAN AUTHORED, WHICH IS WHY THIS CONSTANT IS ONLY THE
 * TAIL OF IT.** The corridor is 44 units deep behind the jellyfish and 18 behind a hull at the far
 * end of its drift; one fuse sized for the first spends two and a half times what the second needs,
 * on every lash of every fight. `throwTail` computes the climb and adds this — the half second of
 * lingering at the wall itself that stops a lash winking out on the exact line the ship is sitting
 * on, which would be a bullet that never quite arrives.
 */
const TAIL_LINGER = 30;

/**
 * How full the hostile pool may be before a boss skips laying its wake, as a count of shots alive.
 *
 * ── THE WAKE IS THE LOWEST-PRIORITY THING ON THE FIELD, AND SAYING SO IS THE WHOLE RULE ─────────
 *
 * ⚠️ **ONE FIGHT IN THE GAME CANNOT AFFORD A WAKE AT ALL AND IT IS NOT A CADENCE PROBLEM.** The frost
 * ship's last phase at `burn` already stands at **111 of the 126** `tests/crowd.test.ts` allows,
 * before this decision touches anything — its own shards, two bolts and twelve flakes apiece (0263).
 * Seven more takes it to 140, and stretching the cadence does not help: at 54, 72, 80 and 90 steps it
 * measured 140, 135, 133 and 133. The peak is a BURST that coincides with the shatter, not a
 * steady-state accumulation, so slowing the wake down cannot reach it.
 *
 * ⚠️ **SO THE ANSWER IS A PRIORITY AND NOT A NUMBER PER BOSS.** A wake exists to make a corner of the
 * screen expensive; a volley exists to be dodged. Where the pool is already carrying a fight, the
 * wake stands aside — which is what `src/sim/pool.ts` would do anyway, except that it would do it to
 * whichever spawn happened to come last, and that is as likely to be the attack.
 *
 * ⚠️ **80, AND IT BINDS ON ONE FIGHT.** Measured over all fourteen bosses, every phase and every
 * tier: with the wake yielding at 80 the frost ship's last phase at `burn` comes back under the line
 * and **nothing else in the game changes at all**, because nothing else is carrying eighty hostile
 * shots when its wake is due. 100 was tried first and left it at 130.
 *
 * ⚠️ **A SKIPPED LASH IS SKIPPED, NOT DEFERRED.** `tailIn` has already been reset, so the wake picks
 * up on its own clock rather than firing a backlog the moment the shatter clears — which would put
 * the burst back exactly where the pool could not carry it.
 */
const TAIL_YIELD = 80;

/**
 * How close to the wall the ship is pinned against a lash may be laid, in world units.
 *
 * ── THE ONE PLACE THE HULL'S OWN BACK IS NOT WHERE THE LASH COMES FROM ──────────────────────────
 *
 * ⚠️ **THE SERPENT'S TAIL IS TEN UNITS FROM `PLAYER_LEAD` AND NOTHING THROWN FROM THERE CAN BE
 * FAIR.** It is the longest body in the game — 0277 put its skull 24 units in front of its centre,
 * so its tail is 24 behind — and at the far end of its drift that leaves a ship parked at the wall
 * about ten units away, of which the two hurtboxes are half. Measured, a lash reached the parked
 * ship in **36 steps**, against the 45 every other threat in the game gives.
 *
 * ⚠️ **AND NO SPEED FIXES IT, WHICH IS WHY THIS IS A STANDOFF AND NOT A SMALLER `TAIL_SPEED`.** The
 * deepest corridor in the game is the jellyfish's 44 units and the shallowest is this 10: one speed
 * slow enough to give three quarters of a second across ten units needs 440 steps to cross
 ...
 *
 * ⚠️ **18 = 45 steps at `TAIL_SPEED`, plus the two hurtboxes and the unit of clearance the guard
 * measures to.** It moves nothing on thirteen of the fourteen hulls, and on the serpent it moves the
 * lash's origin about five units at the far end of a five-unit drift — a twelfth of the length of the
 * animal it is laid behind, which is not a thing that can be seen.
 */
const TAIL_STANDOFF = 18;

/**
 * How fast a tail shot travels, in world units per step.
 *
 * ⚠️ **AN ABSOLUTE, LIKE EVERY OTHER SPEED IN THE GAME** —
 * `docs/decisions/0034-a-threat-is-absolute-and-a-pool-is-the-pairing.md`, which `src/content/shots.ts`
 * states at length. A share of the thrower's own bullet was the first draft and it makes the one
 * quantity this number exists to hold — how long the player has — a function of which boss is
 * throwing.
 *
 * ⚠️ **AND IT IS SOLVED FROM THE FAIR-WARNING FLOOR RATHER THAN CHOSEN.** The tightest corridor in
 * the game is the serpent's, because it has the longest tail: `PLAYER_LEAD` 167.1 against a station
 * of 128 + 5 of drift + the 24 units its `muzzle` puts its skull in front of its centre, so its tail
 * throws from 157 and a ship pinned at the end of its box is **10.1 units away**. Three quarters of a
 * second is the reaction `tests/crowd.test.ts` holds every other threat in the game to and the fuse
 * 0263 gives a shard; 10.1 / 45 steps is 0.224, and this is that rounded down.
 *
 * ⚠️ **AND THE TIER DOES NOT SCALE IT**, on 0270's own precedent: a tier that made this quicker would
 * take a fight below the fair-warning floor the number is solved from. What the tier already does is
 * send the volley more often (`fireGapFor`), and the tail comes with the volley.
 *
 * ⚠️ **SLOW IS NOT A COMPROMISE HERE, IT IS THE PICTURE.** At this speed the shots linger in the
 * corridor for about a second and a half before the view leaves them behind, so standing behind a
 * boss is a place that fills up rather than a place you are sniped in — which is the difference
 * between *expensive* and *walled off* the report asks for.
 *
 * ⚠️ **EXPORTED FOR ONE READER, AND IT IS A SELECTOR RATHER THAN AN ASSERTION.** `tests/back.test.ts`
 * uses it to tell the lash apart from the volley in front of it — a hostile shot climbing up-lane at
 * exactly this rate is one of these and nothing else is — and then asserts in SECONDS, which is the
 * quantity 0027 asks for. Without it the fair-warning claim would be made about every bullet in the
 * game, including rings that have always crossed the corridor at full speed and are not this
 * decision's to slow down.
 */
export const TAIL_SPEED = 0.22;
...
export function stepBoss(
  ...
  */
  ...
  /*
    ── THE LASH OUT OF ITS BACK, ON ITS OWN CLOCK — 0281 ───────────────────────────────────────────

    ⚠️ **NOT PART OF A VOLLEY, AND THE FIRST DRAFT MADE IT ONE.** Hung off `throwAttack`, the wake
    inherited the phase's cadence and therefore the tier's: `fireGapFor` roughly halves the gap at
    `burn`, so the lash arrived twice as often exactly where the hostile pool was already tightest.
    Measured, that took **eleven of the fourteen fights to a full 150 of 150** hostile shots alive,
    where `src/sim/pool.ts` silently drops the next volley — against a baseline where only one phase
    of one fight passed a hundred at all. A wake is a property of the hull being there, not of what it
    is throwing, and its budget has to be its own.

    ⚠️ **AND HANGING IT HERE CLOSES THREE HOLES THE ARM-BY-ARM VERSION LEFT OPEN**: a `beam` phase and
    a `summon` phase throw no bullets at all and so could carry no lash, and a `rain` phase's
    lightning never leaves the body. All three lay a wake now, because all three are a hull standing
    in front of a player who can fly behind it.

    ⚠️ **BELOW THE BARE RETURN, WHICH IS DELIBERATE.** 0150's eye is *the phase that stops shooting*,
    and a boss that kept laying a wake through its own window would be softening the one moment the
    fight hands the player outright — 0255's argument, unchanged.
  */
  boss.tailIn--;
  if (boss.tailIn <= 0) {
    boss.tailIn = TAIL_EVERY;
    // A wake stands aside where the pool is already carrying a fight — the constant has the argument.
    if (shots.size <= TAIL_YIELD) throwTail(bullet, bulletKind, boss, row, shots, cameraAlong, scrollPerStep);
  }
...
/**
 * The lash a boss lays behind itself — 0281. A fan of slow shots up-lane out of the hull's back,
 * swept a little each time so it has no fixed safe bearing.
 *
 * ⚠️ **IT LEAVES THE HULL'S OTHER END, WHICH THE ROW ALREADY NAMES.** 0277 put `muzzle` on the row so
 * a serpent spits from its skull rather than from the middle of its body; negated, that same point is
 * the tail. A row with `muzzle: null` throws from its centre both ways, which is right for the two
 * hulls that have no face — a spinning wall has no back either.
 *
 * ⚠️ **THE SHOT IS THE ROW'S OR THE PHASE'S, NEVER A HEAD'S.** `stepBoss` calls this before it
 * resolves an attack, so a hydra's wake is the hydra's own ink rather than whichever of five heads
 * happens to be up. A wake belongs to the animal, not to the mouth it last used.
 *
 * ⚠️ **Nothing allocates**, on this file's own terms: numbers, and a loop with no array in it.
 */
function throwTail(bullet: ShotRow, kind: number, boss: Entity, row: BossRow, shots: Pool<Entity>, cameraAlong: number, scrollPerStep: number): void {
  // The hull's other end: 0277's muzzle, negated — held off the player's own wall by `TAIL_STANDOFF`,
  // which binds on the serpent alone and is the only reason this is not simply the tail.
  const tail = boss.along - (row.muzzle?.along ?? 0);
  const limit = cameraAlong + PLAYER_LEAD - TAIL_STANDOFF;
  const along = tail < limit ? tail : limit;
  const across = boss.across - (row.muzzle?.across ?? 0);
  // The sweep: a sawtooth wrapped into `TAIL_SWING` behind the hull, on the tail's own counter.
  boss.tailAt += TAIL_TURN;
  const swing = (((boss.tailAt % TAIL_SWING) + TAIL_SWING) % TAIL_SWING) - TAIL_SWING / 2;
  // Straight up-lane, swept. `0` is up-lane exactly as `π` is down-lane, which is the volley's centre.
  const back = swing;
  /*
    ⚠️ **THE FUSE IS THIS BOSS'S OWN CORRIDOR AND NOT A CONSTANT.** How long a lash can possibly
    matter is how long it takes to climb from where it was laid to the wall the ship is pinned
    against — 200 steps behind the jellyfish, which stands furthest down-lane, and 80 behind a hull
    at the far end of its drift. A flat fuse sized for the first spends two and a half times what the
    second needs, every lash, on every one of the fourteen fights: measured, that is what took the
    frost ship's last phase to 140 hostile shots alive against the 126 `tests/crowd.test.ts` allows.
  */
  const life = Math.ceil((cameraAlong + PLAYER_LEAD - along) / TAIL_SPEED) + TAIL_LINGER;
  const step = TAIL_SPREAD / (TAIL_SHOTS - 1);
  const first = back - TAIL_SPREAD / 2;
  for (let i = 0; i < TAIL_SHOTS; i++) {
    const shot = shots.spawn();
    // A lash that will not fit is dropped rather than grown, exactly as the volley in front of it is.
    if (shot === null) break;
    const angle = first + step * i;
    reset(shot, along, across, bullet, kind);
    shot.velAlong = Math.cos(angle) * TAIL_SPEED + scrollPerStep;
    shot.velAcross = Math.sin(angle) * TAIL_SPEED;
    // The fuse. `stepEntities` counts it down and retires the shot; `fissionShots` draws the melt on
    // the step before, so a lash disperses rather than vanishing — 0036.
    shot.lifeFor = life;
    /*
      ⚠️ **A LASH NEVER SHATTERS, AND ITS STAGE IS SPENT AT THE MUZZLE TO SAY SO.** `fissionShots`
      skips a shot whose `turnsLeft` has reached the end of its row's stages, so this is the row's own
      mechanism rather than a flag beside it. A wake is a haze the animal leaves, not a payload.

      ⚠️ **AND WITHOUT IT THE FROST SHIP ALONE COSTS MORE THAN THE OTHER THIRTEEN PUT TOGETHER** —
      0270's rule reaching a place 0270 could not have named. A frost shard is two bolts and then
      twelve flakes, so seven out of the tail is eighty-four bullets nothing budgeted for:
      `tests/crowd.test.ts` went red at a full **150 of 150**, and at **136** with a single shard.
      Spending the stage instead is the whole answer, and it costs the frost ship nothing that the
      other thirteen keep.
    */
    shot.turnsLeft = bullet.fission.length;
  }
}
